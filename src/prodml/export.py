...
def export_and_verify(
    ...
    X = predictor.vectorizer.transform(feature_dicts).astype(np.float32)
    logger.info("features_vectorized", rows=X.shape[0], columns=X.shape[1])

    logger.info("onnx_export_started")
    dummy_input = torch.from_numpy(X[:1])  # convert to tensor
    try:
    ...
    pred_mlp = predictor.model.predict(X)
    pred_ort = np.asarray(session.run(None, {input_name: X})[0]).reshape(-1)
    max_difference = float(np.max(np.abs(pred_mlp - pred_ort)))
